...
基于[`getx`](https://github.com/jonataslaw/getx) 搭建的`flutter` 项目，适用于中大型项目的开发。
...
```
Flutter 2.5.x • channel stable • https://github.com/flutter/flutter.git
Tools • Dart 2.14.x
```
...
此目录为路由文件，此项目的路由方式约定为`命名路由`，为固定目录，目录结构如下：
...
```
├── services.dart
└── user.dart  // 关于用户的API服务
└── home.dart // 首页相关的API服务
```
...
我们应该将发起请求，放在`onInit`钩子里面，例如进入楼盘页面时，我们应该获取楼盘信息，就如同在 `stateful wdiget` 里面的`init`钩子一样。
...
这个`List`对应的假设是楼盘列表，当我们点击列表中某个楼盘时，我们通常会进入到楼盘详情页面，所以我们此时应再添加一个详情页面：
...
### 常用Widget
- Container 是最常用的控件，Container是单容器类控件，即只包含一个子控件，Container可以装饰和定位子控件，例如设置背景颜色、形状等
- Row Row控件水平布局 可控制对齐方式
- Column Column控件垂直布局 可控制对齐方式
- Expanded Expanded填满Row, Column的剩余空间
- ListView 滚动列表
- Padding 提供设置内边距的组件
- Stack 可以将子组件叠加显示，根据子组件的顺利依次向上叠加
- Positioned 包裹的子组件就是定位的子组件，叠加后定位到具体位置
- Image 图片
- Text 文本
- SizeBox 具有固定宽高的组件，直接指定具体的宽高，比如设置一些间隙
